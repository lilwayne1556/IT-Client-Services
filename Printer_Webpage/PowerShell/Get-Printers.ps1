function Get-Printers
{
	...
	(
		...
		[string]$filterNameBy = "SCC*"
	)
	#Query Server for printers
	$printers = Get-Printer -ComputerName $ComputerName | where { $_.Name -match $filterNameBy }
    $printers += Get-Printer -ComputerName puffin | where { $_.Name -match "ITS-US-DELL*" }
	$ports = Get-PrinterPort -ComputerName $ComputerName
    $ports += Get-PrinterPort -ComputerName puffin
	#Generate mappings based on portnames
	$mappings = @()
	foreach ($printer in $printers) {
		foreach ($port in $ports) {
			if ($port.Name -like $printer.PortName -and $mappings.PrinterName -notcontains $printer.Name)	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDASOSdpmCzYdy+
# rjVMEnGO56KIkY8PU1Z3aEf7/y8XnqCCEp8wggPHMIICr6ADAgECAhAiSLvsHK6q
...
# BAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgjsYU/io+RXzXitiTYURbmVfkn+nHqK34
# KwD6/R8zjx8wDQYJKoZIhvcNAQEBBQAEggEApxAC+HrN6bWslm++JKHPfEekK/wv
# oKlR6vk2yVzUQbN43xX/A5coZ49PdsaZtTPLxOJRCSdPpULPi0JufHcifjf+SYv4
# PkwPG2A04NGeMTHc6dNy1BEDf5aNGcG3AgTLQxK6MW6CMPN/Ct1dkvOhcA+xRU6h
# s10wWwrW1qIPNH1Dwx+iltSzE2r+TqQezYVUPbKDaqKvsXIooqzuwQrJC6V1pI5+
# HAsla2vOGBkZ95qEJQRrCh0pboua5wCgtgdazwfXFY7pGsac/P57qR/uBJztkldq
# CWN4MlmVecXNu9rRtHq2TGHRod49KtJJjYbl3eXG3/qTivTbm+U5AEz3K6GCArkw
...
# MQ8XDTE5MDEwNDE1NDYxNlowLwYJKoZIhvcNAQkEMSIEIOfuliLOf9MNsjUyRqGo
# lH0sUZvROaVJ5FwHpU1hEDrGMIGgBgsqhkiG9w0BCRACDDGBkDCBjTCBijCBhwQU
...
# AQEFAASCAQDAJMj8OdfPLu2VT+J5zLbpmpoXU0lUJL8XhGQ4Dd4+5LRYVFiShLkr
# NkaGbed26lzMkcvYDp511TTbTatgChItn6DDLw9eZS3TSErWJ6R6nWhMcXtzlS4b
# aDofQkijZLzCl5YccrFuAqwznSJ8f1zvlHHQFsZtIrPIeKalTZOHOuXHS+WC4v07
# cW2WmvILCKKW7cgri9VgO/yVLSQHCo6py/jo912vRkDogYGioMwIBzfBEjaHLOCn
# m7oIjb5Gdju5hoezjvauYYbK7IrMz+G0SSGdfwM6e7+hD6S9IcW1hjOzrdaoejzi
# urv9QOjz+IqFDY7adVZrOwH19uMkKUBr
