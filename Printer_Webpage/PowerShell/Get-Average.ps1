function Get-Average($Printer, $Desc, $CurrentLevel, $MaximumLevel){
    
    # Support for custom descriptions
    $sub = 0
    if($Desc.length -lt $CurrentLevel.length){
        $sub = $CurrentLevel.length / 2
    }

    # $Desc.length/2 because we don't care about the name of the address, just the value
    for($i = $CurrentLevel.length / 2; $i -lt $sub+$Desc.length; $i++){
        if($MaximumLevel[$i] -eq 0){
            continue
        }

        $Printer | Add-Member -MemberType NoteProperty -Name $Desc[$i-$sub] -Value (($CurrentLevel[$i] / $MaximumLevel[$i]))
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCA83NlUT523se+
# Bj1wFxT6L+bj4QeJlNLgN9TOy6571qCCEp8wggPHMIICr6ADAgECAhAiSLvsHK6q
...
# BAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQg1mDAHgWnnDgogXgjz2WZ8n6vV5yXjT2B
# mkrrXgv9HPkwDQYJKoZIhvcNAQEBBQAEggEAHOcJmzYToFsXwcik61I/wbwnIROW
# 9BWkKLcSI+CrnJ1LbVCrWCS0BZNw8gDxJSOWmgOiNZq3WHU7dMsVUluMczsvnwwM
# 9Td2PrTX4614I9CPk1JM5iRFjIxz521fMfIMFqUsq2tnyUDrrbf7HR4IIlHyehlH
# jGHaPMy0IaDr28FSVuHbNjjzfY2/4DtJ2onZCyPdvmQQWFcJNkk7R0kWXfwQA16t
# jVnODH7kL1B3xaAPwsMqG3YB1GmdkY/s7n5sHxq6yMnzF+08voWQEdsHoLaMXuf/
# F8DdCwVq+W1G7RVkg2si4K9GeS6JLLpwwPGYvubKW3jZtVAWffjOwfx7KqGCArkw
...
# MQ8XDTE5MDEwNDE1NDYxNFowLwYJKoZIhvcNAQkEMSIEIP32hPjeRF1cvhx2kJ/k
# zbua251WZkSTz6M8zRQMcE1AMIGgBgsqhkiG9w0BCRACDDGBkDCBjTCBijCBhwQU
...
# AQEFAASCAQBNrkMU3+xPxcfSs/+zMhhzv24JIPt/ivCfZxZEb+MH0tyNb1OfGtRA
# d3MrO9SLlHzAFTqse1GkbhsOhEjE/1OWLrd4VVz+o9+iJVuqUH8PwL2Gh2HRbWua
# vb4PVJNGWKominP9fRsZDiUQ0U1oxEMAuyw2QzKImGwwhYiQn2UTOkv28JIJXUdf
# I7r0RhUObh1/8ag0/2r9ffYFWknIrqST7HqYU+frVDK/EvoZZ2hEqhFEqPGsV6Wc
# AX+ycQ9zDuXLsBjPMk5677xdaC8jiQBDpa9b2+29hBrehANZH8DyxwEVQMAtgPlu
# fsW73ApMl4BX0QnlQkdWKa9gA6S26oyL
