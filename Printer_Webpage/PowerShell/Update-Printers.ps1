...
while($True){
	...
	Get-Printers -ComputerName penguin | ForEach-Object {
		...
		$Pages = 0
		for ($i = 9; $i -lt $PaperCount.Length; $i++)
		{
			$Pages += $PaperCount[$i]
		}
		$Printer | Add-Member -MemberType NoteProperty -Name "Pages Printed" -Value $Pages
		
		# Size on Cart
        $SizeOnCart = Get-Supplies -Address $_.Address -TreeAddress .1.3.6.1.4.1.674.10898.100.6.4.4.1.1.17.1
        $Printer | Add-Member -MemberType NoteProperty -Name "Sides On Cart" -Value $SizeOnCart[4]

        $Printers += $Printer
    }

    $Printers | ConvertTo-JSON | Out-File "../public/data/printers.json" -Encoding ASCII
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCdUfDuh97qoC7Z
# lkJx4fUVDir4ViSxwHvX5zTC95JC1aCCEp8wggPHMIICr6ADAgECAhAiSLvsHK6q
...
# BAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgca7dARdoA8sa5fZZmqYUw0RO8msY7pSu
# WfjLErmlqBcwDQYJKoZIhvcNAQEBBQAEggEAH56uDceAQyTdYSkWsDgebphjWWTK
# s5EfQDg5hGzbfucGlHIfBwiHpiHtO2KP/XZG0DMzr7c89M/2vJM1eo59DUPj2hR7
# Z3U6M57pG0X+rXEYb0LDo7VoAF6rmLRF6ssliv9QQpS9VECc1IAdJuimUSZDqy+/
# IO2JtK26c71IUg0IwAC7vIzMROMMzIY/13qVgDBtM74EjpJ2YthUBxBbKUx32c0w
# 5NG5IMUoi8OePpUZV4WEku8q3JOMJwe/VSV77qJxGYhvXRtpNobw/iEakdcTDoww
# sw1b21fOGhc5pL+TVZ2h6o342nGnVjYf15GLJG+ZiL3Sj0joxqjJeeK7cqGCArkw
...
# MQ8XDTE5MDEwNDE1NDU1NlowLwYJKoZIhvcNAQkEMSIEIB6lZuwJ4tsCZ0zi2kRZ
# 3ncsaJHAk5rc5xBtY2dxC1IwMIGgBgsqhkiG9w0BCRACDDGBkDCBjTCBijCBhwQU
...
# AQEFAASCAQB3izS7DAyX443uB/PqUaXI4IqZ0NxrjHPH24327Ro4JqKzPt+g4+xI
# nAINxzHNqVUu8IbXnaya3b5QrZie+1xFfvv/mxp97HQghfHsIJL1gNhh2C8DV0Ad
# 6LIUsB7o1nk0ENlwvczwmObSGQmp6culVQfDTB+t7qtIdlEIyk4MAbLZLxwYEyjh
# y7gzWjwxD5TAJ+QKg9CV1+zKZcU9ZEKUJc1ctiaRudZBaefgSn/9COMT5uFpABX4
# HpkmThUuLIjsuWjlzJdFijolicJFxoRNgRqfpCCOfLYJGvnRZhUtN1yUJNy/pR1+
# o/vdhweRRzvdKD6XAMFEComa9wIOY0Tc
