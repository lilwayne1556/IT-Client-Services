...
Function Start-CMClientAction
{
    ...
    Process
    {
        ...
        If (!($PSBoundParameters.Keys.Contains('SCCMActionsBundle')))
        {
            ...
            {
                $sccmcommand = Invoke-Command -ComputerName $Computer -ScriptBlock { Invoke-WmiMethod -Namespace root\CCM -Class SMS_CLIENT -Name TriggerSchedule -ArgumentList $ClientAction }
            }
        }
